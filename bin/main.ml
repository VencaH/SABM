...
module CreateTests (Inputs : TestSettings) : TestModule = struct
  let iteration = match Inputs.iterations with Some x -> x | None -> 1000
  ...
  let step =
    ...
    | Some x ->
        x
    | None ->
        (min_temp /. max_temp) **. (1. /. Float.of_int iteration)
  ...
  let random_search (size : int) (dimensions : int) (min : float) (max : float)
    ...
    inner_rs (size - 1) dimensions min max cost_function start_cost start result
  ...
  let metropolis (t : float) (best : float list) (best_cost : float)
      (current : float list) (current_cost : float) : float * float list =
    let diff = current_cost -. best_cost in
    if Float.( < ) diff 0. then (current_cost, current)
    else
      let prob = 1. /. Float.( ** ) euler (diff /. t) in
      if Float.( > ) prob (Random.float 1.) then (current_cost, current)
      else (best_cost, best)
  ...
  let rec random_local (dim_min : float) (dim_max : float) (current : float) =
    let box_muller () =
      ...
      Float.sqrt (-2. *. Float.log x) *. Float.cos (2. *. y *. pi)
    in
    let rec increment () =
      let inc = box_muller () *. ((dim_max -. dim_min) /. 60.) in
      if Float.( < ) inc (dim_min /. 10.) || Float.( > ) inc (dim_max /. 10.)
      then increment ()
      else inc
    in
    let new_value = current +. increment () in
    if Float.( < ) new_value dim_min || Float.( > ) new_value dim_max then
      random_local dim_min dim_max current
    else new_value

  let local_next (previous : float list) (min : float) (max : float) :
      float list =
    let generate = random_local min max in
    List.map previous ~f:generate

  (* evaluate all local points  *)
  let eval_local (size : int) (start : float list) (t : float) (min_t : float)
      (step_t : float) (min : float) (max : float)
      (cost_function : float list -> float) (result : (float * float list) list)
      : (float * float list) list * float * float list =
    let rec inner_eval_local (size : int) (t : float) (min_t : float)
        (step_t : float) (min : float) (max : float) (cf : float list -> float)
        (best : float list) (best_cost : float)
        (result : (float * float list) list) :
        (float * float list) list * float * float list =
      match size with
      | 0 ->
      ...
      | x ->
          ...
          in
          let result = List.append result [(best_cost, best)] in
          inner_eval_local (x - 1) t min_t step_t min max cf best best_cost
            result
    in
    inner_eval_local size t min_t step_t min max cost_function start
      (cost_function start) result

  (* main SA function  *)
  let simulated_annealing (local_size : int) (max_t : float) (min_t : float)
      (step_t : float) (dimensions : int) (min : float) (max : float)
      (cost_function : float list -> float) =
    let rec inner_sa (local_size : int) (t : float) (min_t : float)
        (step_t : float) (min : float) (max : float) (cf : float list -> float)
        (best : float list) (best_cost : float)
        (result : (float * float list) list) :
        (float * float list) list * float * float list =
      ...
      | t ->
          ...
          inner_sa local_size new_t min_t step_t min max cf best best_cost
            result
    in
    let start = random_params min max dimensions in
    let start_cost = cost_function start in
  ...
  (* run 30 times and collect result  *)
  let result fn =
    let accumulator i acc _ =
      let result, _, _ = fn () in
      let row =
        List.map result ~f:(fun res ->
            let cost, inp = res in
            List.append [cost] inp |> List.append [Float.of_int i] )
      in
      List.append acc [row]
    in
    List.foldi (List.init 30 ~f:(fun _ -> 0)) ~init:[] ~f:accumulator

  (* average progress over 30 runs*)
  let avg_result (result : float list list list) =
    ...
    let accumulator acc res =
      List.map2_exn acc res ~f:(fun a b -> a +. List.nth_exn b 1)
    in
    let sum =
      List.fold result ~init:(List.init len ~f:(fun _ -> 0.)) ~f:accumulator
    in
    List.map sum ~f:(fun x -> Float.( / ) x reps)

  let rnd_fdjn_5 = result (fun () -> random_search 10000 5 (-5.) 5. fst_dejong)

  let rnd_fdjn_10 =
    result (fun () -> random_search 10000 10 (-5.) 5. fst_dejong)

  let rnd_sdjn_5 = result (fun () -> random_search 10000 5 (-5.) 5. snd_dejong)

  let rnd_sdjn_10 =
    result (fun () -> random_search 10000 10 (-5.) 5. snd_dejong)

  let rnd_sch_5 = result (fun () -> random_search 10000 5 (-500.) 500. schwefel)
  ...
  let sa_fdjn_5 =
    result (fun () ->
        simulated_annealing 10 max_temp min_temp step 5 (-5.) 5. fst_dejong )

  let sa_fdjn_10 =
    result (fun () ->
        simulated_annealing 10 max_temp min_temp step 10 (-5.) 5. fst_dejong )

  let sa_sdjn_5 =
    result (fun () ->
        simulated_annealing 10 max_temp min_temp step 5 (-5.) 5. snd_dejong )

  let sa_sdjn_10 =
    result (fun () ->
        simulated_annealing 10 max_temp min_temp step 10 (-5.) 5. snd_dejong )

  let sa_sch_5 =
    result (fun () ->
        simulated_annealing 10 max_temp min_temp step 5 (-500.) 500. schwefel )

  let sa_sch_10 =
    result (fun () ->
        simulated_annealing 10 max_temp min_temp step 10 (-500.) 500. schwefel )

  let sa_fdjn_5_avg = avg_result sa_fdjn_5

  let sa_fdjn_10_avg = avg_result sa_fdjn_10

  let sa_sdjn_5_avg = avg_result sa_sdjn_5

  let sa_sdjn_10_avg = avg_result sa_sdjn_10

  let sa_sch_5_avg = avg_result sa_sch_5

  let sa_sch_10_avg = avg_result sa_sch_10

  let rnd_fdjn_5_avg = avg_result rnd_fdjn_5

  let rnd_fdjn_10_avg = avg_result rnd_fdjn_10

  let rnd_sdjn_5_avg = avg_result rnd_sdjn_5

  let rnd_sdjn_10_avg = avg_result rnd_sdjn_10

  let rnd_sch_5_avg = avg_result rnd_sch_5

  let rnd_sch_10_avg = avg_result rnd_sch_10

  let stats data =
    let len = List.nth_exn data 0 |> List.length |> Int.( + ) (-1) in
    let finals =
      List.map data ~f:(fun l ->
          List.nth_exn l len |> fun l -> List.nth_exn l 1 )
      |> Array.of_list
    in
    [ Owl_stats.mean finals
    ; Owl_stats.median finals
    ; Owl_stats.std finals
    ; Owl_stats.max finals
    ; Owl_stats.min finals ]
  ...
  let print_line fn_name dim data =
    Stdlib.Printf.printf
      "|  %20s |  %2i |  %7.2f |  %7.2f |   %7.2f | %7.2f | %7.2f |" fn_name dim
      (List.nth_exn data 0) (List.nth_exn data 1) (List.nth_exn data 2)
      (List.nth_exn data 3) (List.nth_exn data 4) ;
    Stdlib.print_newline ()

  let print_stats_sa header =
    Stdlib.print_endline header ;
    Stdlib.print_endline
      "|     cost function     | dim |   mean   |  median  | std. dev. |   \
       max   |   min   |" ;
    Stdlib.print_endline
      "-------------------------------------------------------------------------------------" ;
  ...
  let print_stats () =
    Stdlib.Printf.sprintf
      "Statistic values for Simulated Annealing for %s max_temp: %.2f \
       min_temp: %.4f step: %f"
      test_name max_temp min_temp step
    |> print_stats_sa ;
    Stdlib.print_newline () ;
  ...
  let print_comparison () =
    let no_iter data1 data2 =
      List.length data2 |> Float.of_int
    ...
    Plot.plot_fun ~h:out
      ~spec:[RGB (0, 255, 0)]
      (result rnd_fdjn_5_avg) 0. fdjn_5_i ;
    Plot.plot_fun ~h:out
      ~spec:[RGB (0, 0, 255)]
      (result sa_fdjn_5_avg) 0. fdjn_5_i ;
    ...
    Plot.plot_fun ~h:out
      ~spec:[RGB (0, 255, 0)]
      (result rnd_fdjn_10_avg) 0. fdjn_10_i ;
    Plot.plot_fun ~h:out
      ~spec:[RGB (0, 0, 255)]
      (result sa_fdjn_10_avg) 0. fdjn_10_i ;
    Plot.legend_on out [|"Random Search"; "Simulated Annealing"|] ;
    Plot.subplot out 1 0 ;
    ...
    Plot.plot_fun ~h:out
      ~spec:[RGB (0, 255, 0)]
      (result rnd_sdjn_5_avg) 0. sdjn_5_i ;
    Plot.plot_fun ~h:out
      ~spec:[RGB (0, 0, 255)]
      (result sa_sdjn_5_avg) 0. sdjn_5_i ;
    Plot.legend_on out [|"Random Search"; "Simulated Annealing"|] ;
    Plot.subplot out 1 1 ;
    ...
    Plot.plot_fun ~h:out
      ~spec:[RGB (0, 255, 0)]
      (result rnd_sdjn_10_avg) 0. sdjn_10_i ;
    Plot.plot_fun ~h:out
      ~spec:[RGB (0, 0, 255)]
      (result sa_sdjn_10_avg) 0. sdjn_10_i ;
    Plot.legend_on out [|"Random Search"; "Simulated Annealing"|] ;
    Plot.subplot out 2 0 ;
    ...
    Plot.plot_fun ~h:out
      ~spec:[RGB (0, 255, 0)]
      (result rnd_sch_5_avg) 0. sch_5_i ;
    Plot.plot_fun ~h:out
      ~spec:[RGB (0, 0, 255)]
      (result sa_sch_5_avg) 0. sch_5_i ;
    Plot.legend_on out [|"Random Search"; "Simulated Annealing"|] ;
    Plot.subplot out 2 1 ;
    ...
    Plot.plot_fun ~h:out
      ~spec:[RGB (0, 255, 0)]
      (result rnd_sch_10_avg) 0. sch_10_i ;
    Plot.plot_fun ~h:out
      ~spec:[RGB (0, 0, 255)]
      (result sa_sch_10_avg) 0. sch_10_i ;
    Plot.legend_on out [|"Random Search"; "Simulated Annealing"|] ;
    Plot.output out
...
*)
  ...
  let print_sa () =
    ...
    List.iter
      (List.init 30 ~f:(fun x -> Float.of_int x))
      ~f:(fun x ->
        Plot.plot_fun ~h:out
          ~spec:[RGB (Random.int 255, Random.int 255, Random.int 255)]
          (result x sa_fdjn_5) 0. (no_iter sa_fdjn_5) ) ;
    Plot.subplot out 0 1 ;
    Plot.set_title out "SA 1st DeJong with 10 dimensions " ;
    Plot.set_xlabel out "iteration" ;
    Plot.set_ylabel out "CF value" ;
    ...
    List.iter
      ...
      ~f:(fun x ->
        Plot.plot_fun ~h:out
          ~spec:[RGB (Random.int 255, Random.int 255, Random.int 255)]
          (result x sa_fdjn_10) 0. (no_iter sa_fdjn_10) ) ;
    Plot.subplot out 1 0 ;
    Plot.set_title out "SA 2nd DeJong with 5 dimensions " ;
    Plot.set_xlabel out "iteration" ;
    Plot.set_ylabel out "CF value" ;
    ...
    List.iter
      ...
      ~f:(fun x ->
        Plot.plot_fun ~h:out
          ~spec:[RGB (Random.int 255, Random.int 255, Random.int 255)]
          (result x sa_sdjn_5) 0. (no_iter sa_sdjn_5) ) ;
    Plot.subplot out 1 1 ;
    Plot.set_title out "SA 2nd DeJong with 10 dimensions " ;
    Plot.set_xlabel out "iteration" ;
    Plot.set_ylabel out "CF value" ;
    ...
    List.iter
      ...
      ~f:(fun x ->
        Plot.plot_fun ~h:out
          ~spec:[RGB (Random.int 255, Random.int 255, Random.int 255)]
          (result x sa_sdjn_10) 0. (no_iter sa_sdjn_10) ) ;
    Plot.subplot out 2 0 ;
    Plot.set_title out "SA Schwafel with 5 dimensions " ;
    Plot.set_xlabel out "iteration" ;
    Plot.set_ylabel out "CF value" ;
    ...
    List.iter
      ...
      ~f:(fun x ->
        Plot.plot_fun ~h:out
          ~spec:[RGB (Random.int 255, Random.int 255, Random.int 255)]
          (result x sa_sch_5) 0. (no_iter sa_sch_5) ) ;
    Plot.subplot out 2 1 ;
    Plot.set_title out "SA Schwafel with 10 dimensions " ;
    Plot.set_xlabel out "iteration" ;
    Plot.set_ylabel out "CF value" ;
    ...
    List.iter
      ...
      ~f:(fun x ->
        Plot.plot_fun ~h:out
          ~spec:[RGB (Random.int 255, Random.int 255, Random.int 255)]
          (result x sa_sch_10) 0. (no_iter sa_sch_10) ) ;
    Plot.output out
